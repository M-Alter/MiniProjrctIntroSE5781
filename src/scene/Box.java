...
public class Box {
    ...
    public Box(int k, Geometries geometries) {
        ...
        setDensity(geometries.getBodies().size(), k);
        setVoxelSize();
        SetMap(geometries);
    }
    ...
    private void SetMap(Geometries geometries) {
        ...
        Voxel voxel;
        for (Intersectable geometry : geometries.getBodies()) {
     ...
     * @param dis       - the distance from light source
        ...
        while (true) {
            ...
            if (map.containsKey(currentVoxel)) {
                ...
                if (!shadowRaysCase)
                    ...
                    for (Intersectable geometry : voxelGeometris.getBodies()) {
    ...
    private static class Voxel {
        ...
        private boolean isIntersectInVoxelRange(List<GeoPoint> intersections) {
            for (GeoPoint geoPoint : intersections) {
                if (convertPointToVoxel(geoPoint.point3D).equals(this))
                    return true;
            }
            return false;
        }
